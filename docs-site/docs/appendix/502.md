...
> [!NOTE]
> **区分「应用层伪装页」与「平台层拦截」**：在 EdgeOne Makers 上，若访问 `/{adminPath}` 看到 **401 + `Tencent Edgeone` 页**（而非本项目风格的伪装页），那是 **EO 平台「访问保护」** 在应用层之前的鉴权拦截，不是本项目的 502 伪装页，也不是 bug。带有效 `eo_token` cookie 即可进入管理面。详见 [FAQ · EO 管理面 401](/user/08-faq.md)。
