...
网关返回的 502 兜底页由 `src/errorPage.js` 的 `buildErrorPage()` 在**运行时生成**（HTML 字符串，随响应返回），其「仿 Cloudflare 风格」的视觉与随机 Ray ID / 大区文案**借鉴自** `backup/docs/502.html`（原始样例存档），但已做安全改造：
- 展示域名、Ray ID（随机）、大区、时间；
- 标明 Browser/Cloudflare(节点) 正常、**Host(源站) 异常**；
- 提示「几分钟后重试」；
- 去除原样例里 `fetch('https://cloudflare.com/cdn-cgi/trace')` 外泄访客 IP 的逻辑，改为服务端本地随机生成，且内联最小样式、无外部依赖。

> 防盗刷 / 防探测：每次异常返回的 Ray ID 与「大区 - 小区」都是随机伪造，让扫描器误以为是 Cloudflare 边缘在拦截，难以据此判断真实架构。
