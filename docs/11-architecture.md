...
- 顺序固定不可更改（严格遵循 `docs/12-request-flow.md`）。
- 每个环节卡片说明「流量走到这一步在干什么」；有可干预配置项的环节（① 站点 / ② 安全 / ③ 源站 / ④ 规则 / ⑥ cacheGen / ⑧ 源站池）点击可编辑，无配置项的环节（⑤⑦⑨⑩⑪⑫）为纯只读。
- **规则引擎只在 ④ 一个环节**：规则的各类 action 不拆成子环节，而是分别作用到后续真实环节（回源改写→⑧、响应头→⑩、缓存策略→⑪）。
- 单站点：④ 内的规则节点可**拖拽**重排，松手即按新顺序重算 `priority` 并保存（后端按 priority 降序固化）；拖拽只改匹配顺序，不改流程。
