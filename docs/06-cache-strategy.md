...
| **L1 LRU + 上限（256 项 / 16MB）** | 防止内存膨胀，同时保证热点配置常驻、不穿透 | 间接压制冷读 |
| **统计聚合（内存聚合 + 分片键批量落盘）** | 统计写入频次与请求量**解耦**，不每请求写 KV | CF：KV 写次数；EO：KV 空间 |
...
> 管理面（写配置、查 KV 调试）**不经 L1、直接读 KV/Redis**，确保配置变更立即可见——绝不为省额度而让管理面看到旧值。
