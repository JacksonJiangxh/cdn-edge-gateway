...
## 七、免费额度约束与「减少额度触达」（四平台差异 + 本项目克制策略）

> 本项目的核心诉求之一是：**克制有限免费额度使用**，清楚每家的免费额度边界，尽可能减少「读写请求触达」（KV 读次数、函数执行次、回源/源站请求）。
> 链路：浏览器 → 最前端 CDN（CF / EO / CloudFront / ESA）→ 本项目（仅 CF/EO/ESA 走函数层）→ 源站。

### 7.1 四平台免费额度要点（与缓存/额度相关）

| 平台 | 函数执行 | 边缘/节点缓存 | KV / 配置存储 | 关键约束 |
|---|---|---|---|---|
| **Cloudflare** | Workers 免费 10 万次/天；单请求 ≤50 subrequest | `caches.default` 免费，受面板规则权威控制 | **KV 按「操作次数」计费**（免费 10 万次/天）；读计入 subrequest | 重点克制 **KV 读次数**与 subrequest 预算；可缓存内容尽量由 CDN 边缘命中（零函数调用） |
| **EdgeOne** | Makers 免费 300 万次/月（仅算触发次数，KV 读写**不计**函数额度） | 节点本地缓存（不跨节点复制） | KV 仅按**空间**计费（默认 1GB，读写不计次） | 重点克制 **KV 空间**（TTL 别太长、统计键别太多）；函数触发次数宽松 |
| **AWS CloudFront** | 不走本项目（直接源站） | 节点缓存（按请求次数+流量计费） | 无（不经本项目） | 缓存质量靠 CloudFront Cache Policy + 源站头；本项目够不到 |
| **阿里云 ESA** | 函数执行有免费额度与流量限制 | 全局 `cache` 单实例（免费） | **EdgeKV 无免费额度**（按量付费）；故本项目**禁用厂商 KV、改走 `REDIS_URL` 外置 Redis** | 重点：**绝不用 ESA EdgeKV**（成本不可控），配置/统计走外置 Redis（或 KV 仅作可选） |

> 关键结论（与用户原始诉求一致）：**ESA 没有 KV 免费额度**，本项目在 ESA 上已禁用厂商 EdgeKV，统一经 `REDIS_URL` 访问外置 Redis；CF/EO 的 KV 免费额度有限（CF 计次、EO 计空间），必须克制读触达。

### 7.2 本项目已落地的「减少额度触达」措施

| 措施 | 作用 | 省的是什么 |
|---|---|---|
| **数据面 L1 内存缓存（`configCacheTtl`，默认 60s，EO 下限 120s）** | 同一 isolate 内 30s/120s 窗口的配置读命中内存，**KV 冷读次数归零** | CF：KV 读次数 + subrequest；EO：KV 读（虽不计次，但省延迟与空间） |
| **kv.js 并发读合并（in-flight coalescing）** | 同一 isolate 内极短窗口、对同一物理键的并发 `get` 合并为**一次**后端调用 | CF：KV 读次数（冷启动并发、单请求多模块并发读同一配置时显著） |
| **统一内存预算 `memBudget`（128MB 假设）+ 三域收敛** | isolate 级统一内存预算单例，按域（config/stats/ratelimit）分配配额，域超配额自回收、全局逼近硬水位(90%)时强制 trim 到 70% 安全线，**既压低 KV/D1 读写又绝不 OOM** | CF：KV 读/写次数；EO：KV 空间；统一压低所有平台的 KV/D1/RAM 触达 |
| **统计聚合（内存聚合 + 分片键批量落盘）** | 统计写入频次与请求量**解耦**，不每请求写 KV；内存上限由 `memBudget` 动态给出，超配额即丢弃最旧 host | CF：KV 写次数；EO：KV 空间 |
| **限流内存计数（分钟槽）+ `memBudget` 接入** | 高频限流判定走内存，仅远程计数异步同步；内存表上限由 `memBudget` 动态给出，水位触发即清理 | CF：KV/subrequest；EO：延迟 |
| **EO `configCacheTtl` 下限 120s** | 避开 EO KV 跨节点约 60s 最终一致，既压冷读又避旧值 | EO：KV 空间（少写） |
| **ESA 禁用 EdgeKV、走 `REDIS_URL`** | 成本从「按量付费不可控」转为「自有 Redis 可控」 | ESA：避免 EdgeKV 费用失控 |

#### 7.2.1 统一内存预算（memBudget）设计要点

边缘运行时（CF / EO / ESA）的 isolate **都有内存上限**。本项目已确认统一按 **128MB 假设**规划：

- **CF Workers**：标准 128MB 堆上限；
- **ESA 函数侧**：`esa.jsonc` 明确「内存 128MB」（ESA 文档里的 512MB 是企业另一档配额，不在本假设内）；
- 统一以 **128MB** 为上限，可用环境变量 `MEM_BUDGET_BYTES` 在运行时覆盖（如调档到更小/更大的实例）。

> 关键约束：**三平台的 V8 边缘运行时均无 JS 堆内省 API**（浏览器才有的 `performance.memory` 在边缘不可用），因此无法在运行时真实测量堆占用。`memBudget` 改用「**条目数 × 每类平均估算字节（运行采样自校准）+ 条目数硬上限**」双约束近似计量，兼顾准确性与零额外 IO。

三处原有零散内存缓存（配置 L1 / 统计聚合 / 限流计数）已收敛到 `memBudget` 统一管理：

| 域 | 权重 | 回收策略 | 不变量（及时反馈保证） |
|---|---|---|---|
| `config` | 3（最高） | **保守**：仅全局硬水位(90%)才被迫释放；释放只清过期项 | 写操作后 `invalidateMemCache()` 立即失效当前 isolate 内存，下次读直连 KV → **新增/修改/删除配置即时生效、前端渲染永远正确**；TTL 不激进延长、`configCacheTtl=0` 仍保防穿透短窗口 |
| `stats` | 2 | **激进**：超自身配额即丢弃最旧一半 host | 统计可容忍近似丢失（现状已接受「isolate 回收最多丢 500 条或 5 分钟」） |
| `ratelimit` | 1 | **激进**：超自身配额即清过期/全清 | 限流短暂失准但 fail-safe（本地计数兜底），可接受 |

- **配额分配**：按权重把总预算（预留 5% 边距防估算偏低）分给三域；任一域内存表条目上限 = `min(原硬上限, 配额字节 / 估算每条约字节)`，预算缩小时上限自动收紧。
- **自回收**：域超自身配额即 evict 自己（域级水位）；全局逼近 90% 时强制所有域 trim 到 70% 安全线之下，避免平台 OOM 杀 isolate。
- **可观测**：`/__health` 或 `/debug` 响应暴露各域配额使用、估算内存占用（见 `getBudgetSnapshot()`）。

> 管理面（写配置、查 KV 调试）**不经 L1、直接读 KV/Redis**，且写后 `invalidateMemCache` 立即使当前 isolate 失效——**绝不为省额度而让管理面或前端看到旧值**。跨 isolate 的最终一致窗口仍由 `configCacheTtl` 控制（保守短窗口），详见 `docs/11-architecture.md` §4.3。

### 7.3 缓存/回源侧的「减少触达」增益

| 措施 | 行为 | 减少的触达 |
|---|---|---|
| **SWR 响应头（`stale-while-revalidate`）** | `CDN-Cache-Control` 下发 `s-maxage` + `stale-while-revalidate`，边缘在过期窗口内复用旧缓存、后台异步刷新 | 回源请求数、源站压力（CF/EO/ESA 边缘均支持） |
| **serve-stale-on-error**（`pipeline.js` 步骤 7.5） | 回源全失败（5xx/502）时，若边缘仍保留旧缓存则改返回 stale 响应（带 `X-Cache: STALE`），而非把 502 直接暴露 | 用户可见错误、对挂掉源站的无效重试 |
| **被动熔断 + 自愈**（`balancer/circuit.js`） | 某源站连续失败自动隔离（默认 60s），「全员熔断」时放弃过滤避免误杀；冷却后自愈 | 对挂掉源站的反复回源、函数执行次数 |
| **EO 路径 A 同站 fetch 节点缓存** | 无自定义回源 Host 的可缓存请求，由 EO 节点缓存命中（零函数调用） | EO 函数触发次数 |
| **最前端 CDN 锁定 TTL（半年）** | 可缓存内容由 CDN 边缘直接命中，函数零调用 | CF/EO 函数执行次数、subrequest |

> 综合效果：在正常流量下，绝大多数可缓存请求在**最前端 CDN 边缘**即命中（函数零调用、KV 零读）；仅在未命中/不可缓存/鉴权请求才进函数，且函数内配置读命中 L1、源站级故障被熔断/自愈与 stale 兜底吸收。整套链路把免费额度消耗压到最低。
